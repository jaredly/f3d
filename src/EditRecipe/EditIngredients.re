...
let change ingredients ingredientHeaders onChange fn i (ingredient: Models.maybeRecipeIngredient) => {
...
let emptyIngredient () => {"id": uuid(), "ingredient": Models.Text "", "amount": Js.null, "unit": Js.null, "comments": Js.null};
...
let render ::fb ::ingredients ::ingredientHeaders ::allIngredients ::onChange ::onPaste => {
  ...
  let setIngredient (idOrText: Models.idOrText) => change (fun ing => ing##ingredient #= idOrText);
  let setUnit (value: string) => change (fun ing => ing##unit #= (emptyToNull value));
  let setComments (value: string) => change (fun ing => ing##comments #= (emptyToNull value));
  let setAmount (value: option float) => change (fun ing => ing##amount #= (Js.Null.from_opt value));

  let remove i => {
    let ingredients = Array.copy ingredients;
    Js.Array.spliceInPlace pos::i remove::1 add::[||] ingredients |> ignore;
    /** TODO move up headers */
    onChange (Array.map (fun head => head##pos > i ? {"pos": head##pos - 1, "text": head##text} : head) ingredientHeaders, ingredients);
  };
  ...
  let ingredientsAndHeaders = Array.mapi (fun i ing => `Ingredient (i, ing)) ingredients;
  Array.iteri
  (fun i head => {
    let dest = head##pos + i;
    Js.Array.spliceInPlace pos::dest remove::0 add::[|`Header (i, head)|] ingredientsAndHeaders |> ignore
  })
  ingredientHeaders;
  ...
  let addEmptyAfter i => {
    let ingredients = Array.copy ingredients;
    Js.Array.spliceInPlace pos::(i + 1) remove::0 add::[| emptyIngredient() |] ingredients |> ignore;
    onChange (ingredientHeaders, ingredients);
  };
  ...
  let onDrop (key, destkey, isAfter) => {
    ...
    if (not isAfter && k2 === k1 + 1) { () } else {
      ...
      Js.Array.spliceInPlace pos::(k2 > k1 ? k2 - 1 : k2) remove::0 add::[|item|] ingredientsAndHeaders |> ignore;
      Array.fold_left
      (fun (headers, ingredients) item => switch item {
      | `BlankIngredient _
      | `BlankHeader _ => (headers, ingredients)
      | `Ingredient (_, ing) => (headers, Js.Array.concat [|ing|] ingredients)
      | `Header (_, head) => (Js.Array.concat [|{"pos": Array.length ingredients, "text": head##text}|] headers, ingredients)
      })
      ([||], [||])
      ingredientsAndHeaders
      |> (fun a => {Js.log a;a}) |> onChange
    }
  };
  ...
  let renderHeader joinedIndex i pos text isBlank => {
    ...
    <tr
      ...
      ref=?(isBlank ? None : Some(fun node => Js.Null.to_opt node |> optMap (fun node => Js.Dict.set positions key node) |> ignore))
    >
      <td
        onMouseDown=?(isBlank ? None : Some (DragNDrop.startMoving ::positions ::key margin::4 ::onDrop))
      ...
      <td colSpan=7>
        <input
          value=text
          onChange=(fun evt => changeHeader i pos (evtValue evt))
      ...
      <td>
        ...
        : <button onClick=(fun _ => removeHeader i) className=Styles.remove>(str "remove")</button>)
      </td>
    </tr>
  };
  ...
  let renderIngredient joinedIndex i ingredient isBlank => {
    ...
    >
      ...
      <td>
        (isBlank
        ? ReasonReact.nullElement
        : <button onClick=(fun _ => remove i) className=Styles.remove>(str "remove")</button>)
      </td>
    </tr>
  };
  ...
  <tbody>
  (Array.mapi
    (fun i item => switch item {
      | `Ingredient (ii, ingredient) => renderIngredient i ii ingredient false
      | `BlankIngredient ingredient => renderIngredient i (Array.length ingredients) ingredient true
      | `BlankHeader _ => renderHeader i (Array.length ingredientHeaders) (Array.length ingredients + 1) "" true
      | `Header (ii, head) => renderHeader i ii head##pos head##text false
    })
  (Js.Array.concat [|`BlankIngredient (blankIngredient ()), `BlankHeader (0, "")|] ingredientsAndHeaders)
  |> spacedArray (fun i => <tr key=(string_of_int i ^ "s") className=Glamor.(css[height "8px"]) />)
