...
let splitNumbers lines => {
  ...
  (fun count line => Js.String.match_ rx line !== None ? count + 1 : count)
  0 lines;
  [%guard let true = count > 0][@else None];
  let result = [||];
  Array.iter
  (fun line => {
    if (Js.String.match_ rx line !== None) {
...
let splitLines lines initialChar => {
  ...
  (fun line => {
    if (Js.String.startsWith initialChar line) {
      Js.Array.push (Js.String.sliceToEnd from::1 line) result
      |> ignore;
    } else if (result == [||]) {
      Js.Array.push (Js.String.trim line) result |> ignore;
    } else {
...
let chars = [{j|•|j}, {j|•|j}, {j|*|j}, {j|+|j}, {j|•|j}, {j|‣|j}, {j|∙|j}, {j|⋅|j}, {j|◦|j}, {j|⦾|j}, {j|⦿|j}];

let findInitialChar lines => {
  let rec loop chars => switch chars {
    | [] => None
    | [c, ...rest] => {
      let count = Array.fold_left (fun count line => Js.String.startsWith c line ? count + 1 : count) 0 lines;
      if (count > 1) {
        Some c
      } else {
        loop rest
...
let parseBag unit text => {
  let result = maybeFind bagUnits (fun bag => {
    maybeFind (List.assoc bag units) (fun name => {
...
let parseUnit amount text => {
  ...
  let (parenthetical, text) = switch (Js.String.match_ [%bs.re {|/\([^\)]+\)\s/|}] text) {
    | None => ("", text)
    | Some arr => (arr.(0), Js.String.sliceToEnd from::(String.length arr.(0)) text)
  };
  let result = maybeFind units (fun (canon, names) => {
    maybeFind names (fun name => {
      [%guard let true =
        Js.String.startsWith (name ^ " ") text ||
        Js.String.startsWith (name ^ ".") text
      ][@else None];
      let text = Js.String.sliceToEnd from::(String.length name + 1) text |> Js.String.trim;
      let (unit, text) = if (Js.Array.includes canon weightUnits) {
        parseBag canon text
      } else {
        (canon, text)
      };
      let text = Js.String.startsWith "of " text ? Js.String.sliceToEnd from::3 text : text;
      Some (unit, text)
    })
  });
  switch result {
...
let englishInOrder = "one|two|three|four|five|six|seven|eight|nine|ten"
|> Js.String.split "|";
...
let parseIngredient line => {
  ...
  let (num, rest) = switch (Js.String.match_ englishNums line) {
  | Some match => {
    Js.log2 "english" match;
    let num = match.(1);
  ...
  | None => switch (Js.String.match_ numberLike line) {
    | Some match => {
        let rec loop (num, text) => {
          switch (Js.String.match_ [%bs.re {|/^(\d+)\/(\d+)/|}] text) {
            ...
            | None => switch (Js.String.match_ [%bs.re {|/^\d+(\.\d+)?/|}] text) {
              | Some matches => {
...
external maybeFindIngredient: array Models.ingredient => Models.recipeIngredient => (Js.null string, Models.recipeIngredient) = "" [@@bs.val];

let matchIngredient allIngredients ingredient => {
  maybeFindIngredient allIngredients ingredient
  |> (fun (guess, ingredient) => {
    Models.maybeRecipeIngredient guess::?(Js.Null.to_opt guess) ingredient;
  })
};

let parseIngredientsText allIngredients text => {
  let text = switch (Js.String.match_ [%bs.re {|/ingredients\b/i|}] text) {
    | Some [|match|] => Js.String.sliceToEnd from::(String.length match) text
    | Some _ => failwith "Bad match"
    | None => text
  };
  let allIngredients = Array.of_list allIngredients;
  splitList text
  |> Array.map parseIngredient
  |> Array.map (matchIngredient allIngredients)
};

/* Instructions section */

external getData: ReactEventRe.Clipboard.t => string => string = "" [@@bs.scope "clipboardData"][@@bs.send];
...
let parseInstructionsText text => {
  ...
  if (items == [||]) {
    ...
    Some (Array.map (fun text => {"text": text, "ingredientsUsed": Js.Dict.empty()}) items);
  }
};

let parseInstructions onPaste evt => {
  let text = getData evt "text/plain" |> Js.String.trim;
  [%guard let Some items = parseInstructionsText text][@else ()];
...
let parseIngredients allIngredients onPaste evt => {
  ...
  [%guard let None = Js.String.match_ [%bs.re {|/^[\d\s]+$/|}] text][@else ()];
  let partsRe = [%bs.re {|/\ninstructions\n/i|}];
  let parts = Js.String.splitByRe partsRe text;
  let (ingText, instText) = if (Array.length parts === 1) {
    (text, None)
  } else {
